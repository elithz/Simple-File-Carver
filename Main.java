...
/**
 * @author  Ningyuan Elith Zhang
 * @email   elithz@iastate.edu or elithzhang@gmail.com
 * @date    12/19/2019
 * @version 0.9.8
 * 
 * Simple sector based file carver based on file signature analysis
 * CPR E 536 Course Project 3
 * Iowa State University, College of Electric and Computer Engineerng
 */

/**
 * Usage:
 To use this tool, put this .java file under the same directory of the sectors files, then compile and run
 this tool will not really carver files, but put sector files in collections that likely belong to the same file.
 this tool will only work when the number of sector files is 899 
 and the sector files have names in format "BLOCKxxxx"
 if you want to use it with other number of files, please modify the "file iterator"
 if you have different sector file name format, please modify the sector name var
 */
import java.io.*;
import java.util.*;
...
public class Main {
    // collections used to store file indexes
    ...
    public static void main(String[] argv) throws Exception {
        ...
        // file iterator, change 899 to any number that fits your need
        // iterate the sector files
        for (int i = 0; i < 899; i++) {
            // change the sector_name as your need
            ...
            // add spaces between each hex byte presentation
            String val = "2";
            ...
            jpgFinder(str, spaceCount, pdfFormatCont, zzcount, zocount, jpgzzCount, i);
            pdfFinder(str, spaceCount, pdfFormatCont, zzcount, zocount, jpgzzCount, i);
            wordFinder(str, spaceCount, pdfFormatCont, zzcount, zocount, jpgzzCount, i);

        }

        // add header or footer into file content collection if they are not there
        ...
        // sort three collections
        Collections.sort(jpg_collection_content);
        Collections.sort(pdf_collection_content);
        Collections.sort(word_collection_content);
    ...
    public static void jpgFinder(String str, int spaceCount, int pdfFormatCont, int zzcount, int zocount,
            int jpgzzCount, int i) {
        /**
         * 
        ...
        if ((str.contains("ff c0") || str.contains("ff c1") || str.contains("ff c2") || str.contains("ff c3")
                ...
                && !str.contains("80 01 89 3c 01 30 00 00 00 00 00 00 00 00 01 00 00 00 01 00 00 00 00 00 00 00 00 00")
                && zzcount < 20)
            jpg_collection_content.add(i);

    }

    public static void pdfFinder(String str, int spaceCount, int pdfFormatCont, int zzcount, int zocount,
            int jpgzzCount, int i) {
        /**
         * 
         * 
        ...
        if (str.contains("6f 62 6a 3c 3c") || str.contains("65 6e 64 73 74 72 65 61 6d")
                || str.contains("65 6e 64 6f 62 6a") || str.contains("3e 3e 73 74 72 65 61 6d") || pdfFormatCont >= 100
                || str.contains("30 30 30 30 30 30 30 30 30"))
            pdf_collection_content.add(i);

    }

    public static void wordFinder(String str, int spaceCount, int pdfFormatCont, int zzcount, int zocount,
            int jpgzzCount, int i) {
        /**
         * 
         *
